...
function check_current_page_tree(header_text, current_webview) {
    ...
    if (header_text === "确认订单") {
        ...
        return { header: header_text, status: "default" };
    } else if (header_text === "密码测试") {
        console.info("密码测试");
        // 特殊的测试模式标识
        return { header: header_text, status: "password_test" };
    } else {
        console.info("测试密码输入");
        // 检查是否有数字键盘（用于测试密码输入功能）
        // 扩大检测范围，不仅仅在webview内查找
        var passwordTestElement = null;
        
        // 方法1: 在当前webview中查找数字
        if (current_webview) {
            passwordTestElement = current_webview.findOne(text("1").algorithm('DFS'));
            if (!passwordTestElement) {
                passwordTestElement = current_webview.findOne(text("2").algorithm('DFS'));
            }
            if (!passwordTestElement) {
                passwordTestElement = current_webview.findOne(text("3").algorithm('DFS'));
            }
            if (!passwordTestElement) {
                passwordTestElement = current_webview.findOne(text("4").algorithm('DFS'));
            }
            if (!passwordTestElement) {
                passwordTestElement = current_webview.findOne(text("5").algorithm('DFS'));
            }
            if (!passwordTestElement) {
                passwordTestElement = current_webview.findOne(text("6").algorithm('DFS'));
            }
            if (!passwordTestElement) {
                passwordTestElement = current_webview.findOne(text("7").algorithm('DFS'));
            }
            if (!passwordTestElement) {
                passwordTestElement = current_webview.findOne(text("8").algorithm('DFS'));
            }
            if (!passwordTestElement) {
                passwordTestElement = current_webview.findOne(text("9").algorithm('DFS'));
            }
            if (!passwordTestElement) {
                passwordTestElement = current_webview.findOne(text("0").algorithm('DFS'));
            }
        }
        
        // 方法2: 在全局范围内查找数字（如果webview内没找到）
        if (!passwordTestElement) {
            passwordTestElement = text("1").findOne(100);
            if (!passwordTestElement) {
                passwordTestElement = text("2").findOne(100);
            }
            if (!passwordTestElement) {
                passwordTestElement = text("3").findOne(100);
            }
            if (!passwordTestElement) {
                passwordTestElement = text("4").findOne(100);
            }
            if (!passwordTestElement) {
                passwordTestElement = text("5").findOne(100);
            }
            if (!passwordTestElement) {
                passwordTestElement = text("6").findOne(100);
            }
            if (!passwordTestElement) {
                passwordTestElement = text("7").findOne(100);
            }
            if (!passwordTestElement) {
                passwordTestElement = text("8").findOne(100);
            }
            if (!passwordTestElement) {
                passwordTestElement = text("9").findOne(100);
            }
            if (!passwordTestElement) {
                passwordTestElement = text("0").findOne(100);
            }
        }
        
        if (passwordTestElement) {
            console.info("[测试] 检测到数字键盘，进入密码测试模式");
            return { header: header_text, status: "password_test" };
        }
...
while (true) {
    ...
    switch (page_info.status) {
        ...
        case "password_test":
            // 密码输入测试模式 - 独立测试模式
            console.info("=== [密码测试模式] 开始独立测试 ===");
            
            // 获取密码键盘坐标并打印
            var coordinates = getPasswordKeyboardCoordinates();
            
            if (coordinates && Object.keys(coordinates).length > 0) {
                console.info("🎯 [测试结果] 成功找到 " + Object.keys(coordinates).length + " 个数字按钮");
                ...
                for (var digit in coordinates) {
                    ...
                    console.info("   数字 " + digit + " -> 边界: left=" + coord.left + ", top=" + coord.top + ", right=" + coord.right + ", bottom=" + coord.bottom);
                                 ...
                                 // 检查是否有足够的数字来输入用户设置的密码
                 var password = password_setting.toString();
                 var canInputPassword = true;
                 var missingDigits = [];
                 ...
                 if (canInputPassword) {
                     console.info("✅ [密码检查] 所有必需数字都已找到，开始模拟输入密码: " + password);
                    ...
                    for (var i = 0; i < password.length; i++) {
                        ...
                        console.info("👆 [点击操作] 第" + (i + 1) + "步: 点击数字 " + digit + " 坐标: (" + coord.x + ", " + coord.y + ")");
                        click(coord.x, coord.y);
                        sleep(800); // 每次点击间隔800ms，便于观察
                    }
                    
                    console.info("🎉 [输入完成] 密码 " + password + " 输入完成！");
                    
                    // 尝试找到确认按钮
                    sleep(1000);
                    console.info("🔍 [搜索确认] 开始搜索确认按钮...");
                    ...
                    if (!confirmButton) {
                        confirmButton = text("确定").findOne(1000);
                    }
                    if (!confirmButton) {
                        confirmButton = desc("确定").findOne(1000);
                    }
                    
                    if (confirmButton) {
                        var confirmCoord = confirmButton.bounds();
                        console.info("✅ [找到确认] 确认按钮坐标: (" + confirmCoord.centerX() + ", " + confirmCoord.centerY() + ")");
                        console.info("👆 [点击确认] 自动点击确认按钮");
                        confirmButton.click();
                        console.info("🎯 [测试完成] 密码输入和确认操作全部完成！");
                    } else {
                        console.warn("⚠️ [未找到] 未找到确认按钮，需要手动确认");
                    }
                } else {
                    console.error("❌ [测试失败] 无法完成密码输入测试，缺少以下数字按钮: " + missingDigits.join(", "));
                }
            } else {
                console.warn("⚠️ [搜索失败] 当前界面未找到任何数字按钮");
            }
            
            console.info("⏳ [等待检测] 等待3秒后继续检测...");
            sleep(3000);
            break;
        case "default":
            // Default logic
